...
##### Пример 5. Маски ввода в составе модуля:
```Java
...
##### Пример 6. Использование произвольной маски
```Java
...
используется интерфейс `ISlotsParser`. Он позволяет сконвертировать строковое представление маски в
набор слотов.
```Java
...
Во втором случае для создания маски из дескриптора понадобится также `ISlotsParser`. Если для дескриптора
указаны оба - массив слотов и raw-маска, то приоритет имеет массив слотов (для создания маски из
такого дескриптора *не* понадобится `ISlotsParser`).
Также `MaskDescriptor` хранит все _дополнительные параметры_ для маски.

#### Пример создания `MaskDescriptor`
...
абстракный класс `AFormatWatcher`. Класс инкапсулирует создание маски и работу с ней. Создание маски не
...
Он устанавливается на watcher методом `AFormatWatcher#setCallback(FormattedTextChangeListener)`.
...
`MaskDescriptor` и (при необходимости) `ISlotsParser`.
...
в декрипторе указана только raw-маска, то для создания маски потребуется предоставить `ISlotsParser`.
Сделать это можно либо в конструкторе `FormatWatcherImpl`, либо с помощью метода
`FormatWatcherImpl#setSlotsParser(ISlotsParser)`.

Следующий код показывает, как создать watcher с raw-маской и отображением `_` на местах для ввода:
```Java
final EditText editText = (EditText) findViewById(R.id.editCustom);
FormatWatcher formatWatcher = new FormatWatcherImpl(
        new UnderscoreDigitSlotsParser(),
